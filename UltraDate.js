/**
 * UltraDate.js v<pkg.version> (http://hrdaya.github.io/UltraDate.js/)
 *
...
(function(root, factory) {
   ...
   * UltraDateの本体
   *--------------------------------------------------------------------------*/
  var UltraDate = function(year, month, day, hours, minutes, seconds, ms) {
   ...
   * Dateオブジェクトの取得
   *
   * @param {UltraDate|Date|String|Number} date 引数を元にUltraDate|オブジェクトの作成
   ...
   *
   * @param {Number} year 年
   *                 undefinedの時は今日のDateオブジェクトから年の取得
   */
  var _getYear = function(year) {
    return year === undefined ? new Date().getFullYear() : _getInt(year);
  };
  ...
  var _getWeek = function(startDay, reqDate, date) {
    ...
    if (isNaN(reqDate) || (reqDate < 0 && reqDate > 6)) {
      reqDate = 4;
    }
    ...
    var lapsedDays = ((thisDate - firstDate) / 86400000) + 1 + reqDate + thisDay;
     ...
     * 該当年の最終週に1を足した値となる
     */
    return reqDate === 0 ? Math.ceil(lapsedDays / 7) : Math.floor(lapsedDays / 7);
  };

  /*----------------------------------------------------------------------------
   * UltraDateのStaticメソッド
   *--------------------------------------------------------------------------*/
  /**
   * Date.now
   *
  ...
  UltraDate.getISOLastWeekNum = function(year) {
    ...
    return date.getISODay() < 4 ? date.addDate(date.getISODay() * -1).getISOWeek() : date.getISOWeek();
  };
  ...
  UltraDate.getHolidays = function(year, locale) {
    ...
    return _holidays.hasOwnProperty(locale) ? _holidays[locale].get(year) : _holidays.def.get(year);
  };
  ...
  UltraDate.setFormatOption = function(locale, options) {
    if (typeof locale !== 'string' || locale === 'def' || locale === '' || Object.prototype.toString.call(options) !== '[object Object]') {
      throw new Error('Data type of the argument is incorrect');
    } else {
      if (!_formats.hasOwnProperty(locale)) {
        _formats[locale] = {};
      }
      if (options.hasOwnProperty('era') && !options.hasOwnProperty('eraStrict')) {
        options.eraStrict = options.era;
      }
      var keys = Object.keys(_formats.def);
      for (var i = 0, len = keys.length; i < len; i++) {
        _formats[locale][keys[i]] = options.hasOwnProperty(keys[i]) ? options[keys[i]] : _formats.def[keys[i]];
      }
    }
    return this;
  };
  ...
  UltraDate.prototype = {
    ...
    format: function(format, eraStrict, locale) {
      ...
      locale = typeof locale !== 'string' || locale === '' ? _defaultLocale : locale;

      var options = _formats.hasOwnProperty(locale) ? _formats[locale] : _formats.def;
      var era = eraStrict === true ? options.eraStrict(this) : options.era(this);
      ...
      var reg = new RegExp('(' + Object.keys(formatting).join('|') + ')', 'g');
    ...
    setDayCountsInMonth: function(count, day) {
      ...
      day = parseInt(day, 10);
      if (isNaN(count) || isNaN(day) || (day < 0 && day > 6)) {
        throw new Error('Data type of the argument is incorrect');
      } else {
        this.setDate(1);
    ...
    setISOWeekDay: function(week, day, year) {
      ...
      if (isNaN(day) || (day < 0 && day > 6)) {
        ...
        year = _getYear(this.getFullYear());
        this.setFullYear(year, 0, 1);
        day = day === 0 ? 6 : day - 1;
        var offset = this.getDay() < 2 || this.getDay() > 4 ? 0 : 1;
    ...
    setUSWeekDay: function(week, day, year) {
      ...
      if (isNaN(day) || (day < 0 && day > 6)) {
        ...
        this.setDayCountsInMonth(0, 0).addDate(((_getInt(week) - offset) * 7) + day);
      }
      return this;
    },
    /**
    ...
    getAge: function(date) {
      var today = date === undefined ? new UltraDate() : new UltraDate(date);
      var birthday = this.copy().clearTime();
      var years = today.clearTime().getFullYear() - birthday.clearTime().getFullYear();
    ...
    diffMonth: function(date) {
      date = _getDate(date);
      return ((date.getFullYear() * 12) + date.getMonth()) - ((this.getFullYear() * 12) + this.getMonth());
    },
    /**
    ...
    diffDate: function(date) {
      date = _getDate(date);
      var diff = date.clearTime().getTime() - this.copy().clearTime().getTime();
      // 24 * 60 * 60 * 1000 = 86400000
